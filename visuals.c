...
//		return 0;
	VectorSubtract(eth.entities[player].origin, eth.cg.refdef->vieworg, vec);
	vec[2] = 0.0f;
	distance = VectorLength(vec) * 0.025;
	vectoangles(vec, rot);
	rot[1] = AngleNormalize180(rot[1] - eth.cg.refdefViewAngles[1]);
...
void drawIrcFireteam() {
	...
	for (; count < (sizeof(ircBuddies) / sizeof(ircBuddy_t)); count++) {
		if (ircBuddies[count].infoValid) {
			...
			if (isOnServer) {
				sprintf(ircBuddies[count].ftString, "^0(^7%s ^1- ^n%d^0)", eth.clientInfo[ircBuddies[count].playerNumber].name, ircBuddies[count].playerNumber);
	...
	for (count = 0; count < (sizeof(ircBuddies) / sizeof(ircBuddy_t)); count++) {
		...
		if (buddy->infoValid) {
			...
			if (isOnServer) {
				qhandle_t flag = 0;
				if (eth.clientInfo[buddy->playerNumber].team == TEAM_ALLIES)
					flag = eth.bg_alliedpin;
				else if (eth.clientInfo[buddy->playerNumber].team == TEAM_AXIS)
					flag = eth.bg_axispin;
				if (flag)
	...
	if (ircMsgsGeneral.lastCheck < (int) eth.cg_time - IRC_CHAT_TIME) {
		if (syscall_CG_Key_IsDown(ETH_IRC_KEY)) {
			if (ircMsgsGeneral.show)
				ircMsgsGeneral.show = qfalse;
			else
				ircMsgsGeneral.show = qtrue;
	...
	if (ircMsgsGeneral.show) {
		...
		while (ircmsgcount >= 0) {
			...
			if (ircMsgs[ircmsgcount].type == IRC_MSGTYPE_CHAN)
				sprintf(ircmsgtext, "^1<^0%s^1> ^7%s", ircMsgs[ircmsgcount].nick, ircMsgs[ircmsgcount].msg);
			else if (ircMsgs[ircmsgcount].type == IRC_MSGTYPE_QUERY)
				sprintf(ircmsgtext, "^d[PM] ^1<^0%s^1> ^7%s", ircMsgs[ircmsgcount].nick, ircMsgs[ircmsgcount].msg);
			else
				ircmsgtext[0] = '\0';
...
void drawIrcTopic() {
	...
	if (seth.value[VAR_IRC_INVITE] && drawLastInvite()) {
		...
		sprintf(str, " ^2%s ^ninvites you to join ^2%s:%i^n - type '^2\\eth_joinbuddy %i^n' to join him! ", buddy->nick + sizeof(IRC_BOT_PREFIX) - 1, buddy->server.hostname, buddy->server.port, id);
	}
	else {
...
void drawEspEntities() {
	...
	for (entityNum=0; entityNum<MAX_CLIENTS; entityNum++) {
		...
		//if ((cv_espdist.integer != 0) && (entity->distance > cv_espdist.integer))
			//continue;
		if (!entity->isInScreen && eth.clientInfo[entityNum].infoValid) //|| !IS_VALID_PLAYER(entityNum))
			continue;

		if (!entity->espColor) continue;
		...
		// Name esp
		#define ESP_NAME_SCALE 0.24f
		char *str = eth.clientInfo[entityNum].name;
			int espvar = seth.value[VAR_ESPNAME];
			switch(espvar) {
		    case 2:
				if (seth.value[VAR_ESPNAME_BG])
			    drawTextRect((entity->screenX - (sizeTextWidth(ESP_NAME_SCALE, str, &eth.fontVariable) / 2)), entity->screenY - 20, 0.19f, str, colorWhite, qfalse, qfalse, qtrue, qtrue, colorBlack, colorLtGrey, &eth.fontVariable);
			else
			    drawText((entity->screenX - (sizeTextWidth(ESP_NAME_SCALE, str, &eth.fontVariable) / 2)), entity->screenY - 20, 0.19f, str, colorWhite, qtrue, qtrue, &eth.fontVariable);
			break;
		    case 1:
				if (seth.value[VAR_ESPNAME_BG])
			    drawTextRect((entity->screenX - (sizeTextWidth(ESP_NAME_SCALE, str, &eth.fontVariable) / 2)), entity->screenY - 20, 0.19f, str, colorWhite, qtrue, qfalse, qtrue, qtrue, colorBlack, colorLtGrey, &eth.fontVariable);
			else
			    drawText((entity->screenX - (sizeTextWidth(ESP_NAME_SCALE, str, &eth.fontVariable) / 2)), entity->screenY - 20, 0.19f, str, colorWhite, qfalse, qtrue, &eth.fontVariable);
			break;
		    case 0:
		    default:
			break;
		...
		int wnum = eth.cg_entities[entityNum].currentState->weapon;
		if (seth.value[VAR_MORTARTRACE]) {
		if(wnum == WP_MORTAR_SET)
		    drawMortarTrace(entityNum);
		}
		//icon esp 2
		    
        if (seth.value[VAR_ESPICON] == 2) { //2nd type here .. first in engine.c
			int cnum = eth.clientInfo[entityNum].cls;
			vec4_t color;
			if (eth.entities[entityNum].espColor)
				VectorCopy(eth.entities[entityNum].espColor, color);
			color[3] = 1.0f;
		    syscall_CG_R_SetColor(color);
		    
		    drawPic( entity->screenX , entity->screenY-10, 15, 15, eth.classIcons[cnum]);
		    drawPic( entity->screenX-(55/2)+30, entity->screenY-25, 25, 15, eth.weaponIcons[wnum]);
...
/*		if (cv_espicon.integer) {
			if(cv_espicon_bg.integer)
			    drawFillRect( entity->screenX-(55/2) - 1 , entity->screenY-26, 57 ,17, ircFTwin.bgColor);
			syscall_CG_R_SetColor(entity->espColor);
*/
//!!! moved to eth_CG_R_AddRefEntityToScene 
//!!! (refEntity_t *refEnt)
//		    if(seth.value[VAR_ESPICON]) {
//			syscall_CG_R_SetColor(entity->espColor);
//			entity->espColor[3] = 1.0f;
		    
//this is gay... nothing is displayed
//			drawPic( entity->screenX , entity->screenY-10, 15, 15, eth.classIcons[eth.clientInfo[entityNum].cls]);
//			drawPic( entity->screenX-(55/2)+30, entity->screenY-25, 25, 15, eth.weaponIcons[eth.clientInfo[entityNum].weapon]);
//			drawPic( entity->screenX-(55/2)+15, entity->screenY-25, 15, 15, eth.rankIcons[eth.clientInfo[entityNum].rank]);
//			syscall_CG_R_SetColor(NULL);
...
/*
	...
	for (entityNum=MAX_CLIENTS; entityNum<MAX_GENTITIES && seth.value[VAR_ESPENT]; entityNum++) {
		...
		// XXX: i changed entity->espColor to colorYellow for now, it was invisible..
		...
		// Dynamite esp
			...
			int time = 30 - ((eth.cg_time - eth.cg_entities[entityNum].currentState->effect1Time) / 1000);
			if (time > 0) {
				char timeStr[8];
...
void drawSpawnTimerRight() {
	...
	char str[5];
    sprintf(str, "%i", getSpawntimer(qtrue));
...
void drawETHhud() {
	...
	sprintf(relStr, "%d%%", ((eth.cg_snap->ps.weaponTime * 100) / eth.reloadScale));
	...
	if (isAimableWeapon()) {
    	if (clips < 5 ) {
	    	if (ammo > 0 ) { // fuck reload then 
				char wtext[100];
				if (!eth.reloading) {
				    sprintf(wtext,"RELOAD NEEDED!(%d)",clips);
				    fxphase.clips_phase++;
	    ...
	    if (clips > 0) {
			...
			for (i = 0; i < strlen(clipStr); i++) 
			    drawPic(WP_X + 5 + (10 * i ), WP_Y + 20 , 10, 15, eth.number[clipStr[i]-48]);
	    }
	    ...
	    if ( ammo > 0)  {
			for (i=0;i<strlen(ammoStr);i++) 
			    drawPic(WP_X + 5 + (strlen(clipStr) * 10 + 10) + (10 * i ), WP_Y + 20, 10, 10, eth.number[ammoStr[i]-48]);
		}
	}
	...
	if (health < 40) {
		...
		if (fxphase.health_phase == 2) fxphase.health_phase = 0;
    ...
    int warnAdd = 0;
    if (enemyStime > -1) {
		sprintf(spawnTimeE, "%d", enemyStime);
    ...
    if (teamStime > -1) {
		sprintf(spawnTimeF,"%d",teamStime);
	...
	// draw kills
    char killsC[50];
    sprintf(killsC,"%d",eth.killCount);
    ...
    if (color) {
		char killMsg[1024];
		sprintf(killMsg,"You killed %s!",eth.VictimName);
...
void drawReferees() {
	if (seth.value[VAR_REFLIST] == REF_LIST) {
		...
		char str[64];
    		sprintf(str, "Referees: ^2%i", seth.refCount);
   		int w = sizeTextWidth(0.19f, str, &eth.fontVariable);
    		int y = 137;
    		int x = 634 - w;
...
int getVarOffset(int var) {
	switch(var) {
		...
		case 4:
			return (int)(refdef_t *)eth.cg.refdef;
		case 5:
			return (int)(float *)eth.cg.refdefViewAngles;
		case 6:
...
void drawMemDump( int offset ) {
	...
	sprintf(d_text,"Memdump for offset: ^3%s ^2( + 0x%.8x)",debugFName[(int)seth.value[VAR_MEMDUMP]],((eth.offsetMul*200)+(eth.offsetSub*20)));
