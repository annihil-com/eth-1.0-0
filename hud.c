...
void addHudConsoleChat(char *msg) {
	...
	while (msgCount >= 0) {
		...
		strcpy(hudChatMsgs[msgCount + 1].msg, hudChatMsgs[msgCount].msg);
		msgCount--;
	}


	hudChatMsgs[0].time = eth.cg_time;
	strcpy(hudChatMsgs[0].msg, msg);
}
...
void addHudConsoleMsg(char *msg) {
	...
	while (msgCount >= 0) {
		...
		strcpy(hudConsoleMsgs[msgCount + 1].msg, hudConsoleMsgs[msgCount].msg);
		msgCount--;		
	}

	hudConsoleMsgs[0].time = eth.cg_time;
	strcpy(hudConsoleMsgs[0].msg, msg);
}
...
void drawHudMisc() {
	...
	if (eth.cg_snap->ps.clientNum != eth.cg_clientNum)
		...
		action = "Playing as";
	sprintf(msg, "^7%s ^2[ ^7%s ^2] [ ^f%s ^2] [ ^f%s ^2]", action, eth.clientInfo[eth.cg_snap->ps.clientNum].name, team, class);
...
void drawHudPowerStats() {
	...
	if ((current > 0) && (eth.cg_snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR)) {
		...
		char cHPstr[30];
		sprintf(cHPstr, "^n%dHP ^2(^n%d%%^2)", current, (int) percentage);
		...
		if (!eth.lastKillTime)
			lastKillTime = 0;
		sprintf(killCount, "^0( ^nKills: ^2%d ^0| ^nLast kill: ^2%d^nsec ago ^0)", eth.killCount, lastKillTime);
		...
		for (i=MAX_HEALTH_POPUPS-1; i>=0; i--) {
			if ((eth.cg_time - hudhpp[i].time) < HEALTH_POPUP_STAYTIME) {
				...
				char popUpMsg[10];
				sprintf(popUpMsg, "%s%d", hpType, hudhpp[i].hp);
...
void drawHudVictimMsg() {
	...
	if ((eth.lastKillTime) && (eth.VictimName) && (stayTime)) {
		if (eth.lastKillTime + stayTime > eth.cg_time) {
			...
			char msg[256];
			sprintf(msg, "^nYou killed ^7%s^n !", eth.VictimName);
...
void addHudOutputMsg(hudicontype type, const char *format, ...) {
	...
	while (msgCount >= 0) {
		if (hudoutput[msgCount].msg) {
			...
			strcpy(hudoutput[msgCount + 1].msg, hudoutput[msgCount].msg);
			msgCount--;
		}
	}

	strcpy(hudoutput[0].msg, msg);
...
void drawHudSystemStats() {
	...
	sprintf(local, "%s ^2@^n %s %s", localTime, eth.mod.name, eth.mod.version);
	...
	if (showFps) {
		char msgCurrent[20];
		sprintf(msgCurrent, "^nFPS(^2%d^n)", hudsystem.fpsCurrent);
		...
		char msgMax[20];
		sprintf(msgMax, "^nMAX(^2%d^n)", hudsystem.fpsMax);
...
void drawHudTimers() { // TODO: round timer (timelimit)
	...
	if (!isSpectator) {
		char str[30];
		sprintf(str, "^0[^1 %d ^2-^1 %d ^0]", getSpawntimer(qfalse), getSpawntimer(qtrue));
...
void drawHudWeaponInfo() { // TODO: better ammo handling (akimbos)
	...
	if (showHudWeaponAmmo(weapon)) {
		...
		if ((clips > 0) || (ammo > 0))
			sprintf(ammoStr, "^n%2d^2/^n%d", clips, ammo);
		else
			sprintf(ammoStr, "^n%d", clips);
