...
qboolean worldToScreen(vec3_t worldCoord, int *x, int *y) {
	...
	AngleVectors (eth.cg.refdefViewAngles, vfwd, vright, vup);
	VectorSubtract (worldCoord, eth.cg.refdef->vieworg, local);
	...
	switch(eth.cg_snap->ps.weapon) {
		...
		case WP_K43_SCOPE:
			xzi = 320 / transformed[2];
			yzi = 240 / transformed[2];
    		break;
		default:
			xzi = 320 / transformed[2] * (90.0 / eth.cg.refdef->fov_x);
			yzi = 240 / transformed[2] * (90.0 / eth.cg.refdef->fov_y);
			break;
	}
